...
> Отдельная благодарность [Sotnikov Maksim](https://github.com/SotnikovMaksim) за тестирование на MacOS и идею с флагом `--no-copy`/`-nc`.

Здесь представлена инструкция, являющийся продолжением [другой инструкции](https://telegra.ph/Kak-podgotovitsya-k-JavaAdvanced-02-16), дополненная лишь ещё несколькими методами запуска преимущественно на операционной системе Windows. Однако скрипты, которые приведенные далее в инструкции, [могут подойти и для других систем](#про-скрипты). Ниже приведены следующие виды запусков на ОС Windows:
...
> Работоспособность скриптов были проверены на Windows 10 (последней версии), Linux (Ubuntu 22.04) и MacOS.
...
4. Добавьте конфигурацию запуска.
    ...
    * Также скрипт поддерживает режим с пятью аргументами, об этом Вы можете прочитать [здесь](#запуск-с-пятым-аргументом---no-copy-nc).
5. Дополнительную информацию по работе скрипта *Test.sh* Вы можете прочитать [здесь](#про-скрипты).
...
5. Запустите терминал/командную строку и установите ***Ubuntu 20.04.6 LTS*** с помощью следующей команды:
    ...
    Альтернативным вариантом Вы можете установить [приложение с Microsoft Store](https://apps.microsoft.com/store/detail/ubuntu-20046-lts/9MTTCL66CPXJ).
...
7. Добавьте в директорию проекта скрипты [`Test.sh`](Test.sh) и [`TestWSL.cmd`](TestWSL.cmd).
